var OrgChart = function(t, e) {
    ...
    e)
        for (var i in this.config)
            "object" != typeof e[i] || Array.isArray(e[i]) ? void 0 !== e[i] && (this.config[i] = e[i]) : this.config[i] = OrgChart._mergeObject(this.config[i], e[i]);
    ...
    null != this.config.nodeMouseClickBehaviour && (this.config.nodeMouseClick = this.config.nodeMouseClickBehaviour),
    null != this.config.mouseScroolBehaviour && (this.config.mouseScrool = this.config.mouseScroolBehaviour),
    OrgChart._validateConfig(this.config) && (this.server = null,
    this._vScroll = {},
    ...
    this.config.dragDropMenuUI ? this.dragDropMenuUI = this.config.dragDropMenuUI : this.dragDropMenuUI = new OrgChart.menuUI,
...
OrgChart.prototype.getByParentId = function(t) {
    for (var e = [], r = 0; r < this.config.nodes.length; r++)
        this.config.nodes[r].pid == t && e.push(JSON.parse(JSON.stringify(this.config.nodes[r])));
    return e
}
,
OrgChart.prototype._draw = function(t, O, v, x) {
    var y = this;
    ...
    this.server.read(t, this.width(), this.height(), e, O, v, function(t) {
        O != OrgChart.action.exporting && (y.nodes = t.nodes),
        y.editUI.fields = t.allFields;
        var e = {
            defs: ""
        };
        OrgChart.events.publish("renderdefs", [y, e]);
        var r = y.ui.defs(e.defs)
          , i = y.getScale(t.viewBox);
        r += y.ui.pointer(y.config, O, i);
        var a = y.getViewBox()
          , o = t.viewBox;
        e = {
            content: r,
        ...
        for (var n in OrgChart.events.publish("prerender", [y, e]),
        r = e.content,
        t.visibleNodes) {
            var l = t.visibleNodes[n]
              , s = y._get(n);
            r += y.ui.node(l, s, t.animations, y.config, void 0, void 0, void 0, O, i, y),
            r += y.ui.link(l, y, y.config, i),
            r += y.ui.expandCollapse(y, l, y.config, O, i)
        }
        e = {
            content: r,
            res: t
        };
        if (OrgChart.events.publish("render", [y, e]),
        r = e.content,
        r += y.ui.lonely(y.config),
        O !== OrgChart.action.exporting) {
            O !== OrgChart.action.centerNode && O !== OrgChart.action.insert && O !== OrgChart.action.expandCollapse && O !== OrgChart.action.update || (o = a),
            O === OrgChart.action.init && null != a && (o = a),
            y.response = t;
            m = y.ui.svg(y.width(), y.height(), o, y.config, r);
            if (y._initialized) {
                var h = y.getSvg()
                  , d = h.parentNode;
                d.removeChild(h),
                d.insertAdjacentHTML("afterbegin", m),
                y._attachEventHandlers(),
                y.xScrollUI.addListener(y.getSvg()),
                y.yScrollUI.addListener(y.getSvg()),
                y.xScrollUI.setPosition(),
                y.yScrollUI.setPosition()
            } else
                y.element.innerHTML = y.ui.css() + y.ui.exportMenuButton(y.config) + m,
                y._attachInitEventHandlers(),
                y._attachEventHandlers(),
                y.xScrollUI.create(y.width(), y.config.padding),
                y.xScrollUI.setPosition(),
                y.xScrollUI.addListener(y.getSvg()),
                y.yScrollUI.create(y.height(), y.config.padding),
                y.yScrollUI.setPosition(),
                y.yScrollUI.addListener(y.getSvg()),
                y.config.enableSearch && y.searchUI.addSearchControl(),
                y.toolbarUI.init(y, y.config.toolbar);
            var c = !1;
            y.notifierUI.show(t.notif);
            var g = y.response.animations;
            if (0 < g[0].length) {
                y._hideBeforeAnimation(g[0].length);
                for (var p = 0; p < g[0].length; p++)
                    g[0][p] = y.getNodeElement(g[0][p]);
                OrgChart.anim(g[0], g[1], g[2], y.config.anim.duration, y.config.anim.func, function() {
                    c || (x && x(),
                    OrgChart.events.publish("redraw", [y]),
                    y._showAfterAnimation(),
                    c = !0)
                })
            }
            O === OrgChart.action.centerNode ? OrgChart.anim(y.getSvg(), {
                viewbox: a
            }, {
                viewbox: y.response.viewBox
            }, y.config.anim.duration, y.config.anim.func, function() {
                y.ripple(v.options.rippleId),
                c || (x && x(),
                OrgChart.events.publish("redraw", [y]),
                y._showAfterAnimation(),
                c = !0)
                ...
                y.xScrollUI.setPosition(),
                y.yScrollUI.setPosition()
            }) : !a || !y.response || a[0] == y.response.viewBox[0] && a[1] == y.response.viewBox[1] && a[2] == y.response.viewBox[2] && a[3] == y.response.viewBox[3] || O !== OrgChart.action.insert && O !== OrgChart.action.expandCollapse && O !== OrgChart.action.update && O !== OrgChart.action.init ? 0 == g[0].length && (c || (x && x(),
            OrgChart.events.publish("redraw", [y]),
            c = !0)) : OrgChart.anim(y.getSvg(), {
                viewbox: a
            }, {
                viewbox: y.response.viewBox
            ...
            y._initialized || (y._initialized = !0,
            OrgChart.events.publish("init", [y]))
        } else {
            var f = t.boundary
              , u = f.maxX - f.minX
              , C = f.maxY - f.minY
              , m = y.ui.svg(u, C, [f.minX, f.minY, u, C], y.config, r, i);
            x(m)
        }
    }, function(t) {
...
OrgChart.prototype.isChild = function(t, e) {
    var r = this.getBGNode(e)
      , i = this.getBGNode(t);
    if (!(r.isGroup || i.isGroup || r.isChildOfGroup || i.isChildOfGroup))
        for (var a = this.getBGNode(r.pid); null != a; ) {
            if (a.id == t)
                return !0;
            a = this.getBGNode(a.pid)
        }
    return !1
}
,
    ...
    if (this._canUpdateLink(t, e)) {
        ...
        if (r.isGroup && i.isGroup)
            for (var a = i.bgnodes[0].id, o = 0; o < r.bgnodes.length; o++) {
                (n = this.get(r.bgnodes[o].id)).pid = a,
                this.update(n, !0)
            }
            ...
            for (o = 0; o < r.bgnodes.length; o++) {
                (n = this.get(r.bgnodes[o].id)).pid = e,
                this.update(n, !0)
            }
        else if (!r.isGroup && !i.isGroup) {
            var n;
        ...
        r.isChildOfGroup && (this.removeNodeTag(r.id, r._groupParentNodeId),
...
OrgChart.prototype.updateNode = function(t, e) {
    var r = this;
    if (!1 === this.update(t, e))
        return !1;
    var i = t.pid;
    return null != i && null != i && "" != i || (i = t.id),
    this._draw(!1, OrgChart.action.update, {
        id: i
    }, function() {
        r.ripple(t.id)
...
OrgChart.prototype.remove = function(t, e) {
    var r = this.getBGNode(t);
    if ((r.isChildOfGroup && this._removeGroupIfHasOneNode(r._groupParentNodeId, r.id),
    !0 === e) && !1 === OrgChart.events.publish("remove", [this, t]))
        return !1;
    for (var i = null, a = this.config.nodes.length - 1; 0 <= a; a--)
        if (this.config.nodes[a].id == t) {
            i = this.config.nodes[a].pid,
...
OrgChart.prototype.addNode = function(t, e) {
    ...
    var i = this.getBGNode(t.pid)
      , a = t.pid;
    return i && i.isChildOfGroup && (a = i._groupParentNodeId),
    this._draw(!1, r, {
        id: a,
        insertedNodeId: t.id
    }),
...
OrgChart.prototype.add = function(t, e) {
    if ((null == t.id && console.error("Call addNode without id"),
    !0 === e) && !1 === OrgChart.events.publish("add", [this, t]))
        return !1;
    return this.config.nodes.push(t),
    !0
}
,
OrgChart.prototype.addTag = function(t, e, r) {
    ...
    if ((i[t] = e,
    !0 === r) && !1 === OrgChart.events.publish("updatetags", [this, i]))
        return !1;
    return this.config.tags = i,
    !0
}
,
...
OrgChart.prototype.removeTag = function(t, e) {
    ...
    if ((delete r[t],
    !0 === e) && !1 === OrgChart.events.publish("updatetags", [this, r]))
        return !1;
    return this.config.tags = r,
    !0
}
,
...
OrgChart.prototype.addNodeTag = function(t, e, r) {
    ...
    -1 == i.tags.indexOf(e) && (i.tags.push(e),
    this.update(i, r))
}
,
...
OrgChart.prototype.group = function(t, e, r, i) {
    ...
    if (a.isGroup || o.isGroup || a.isChildOfGroup || o.isChildOfGroup)
        if (a.isGroup || o.isGroup || !a.isChildOfGroup || o.isChildOfGroup)
            if (a.isGroup || o.isGroup || a.isChildOfGroup || !o.isChildOfGroup)
                if (!a.isGroup && !o.isGroup && a.isChildOfGroup && o.isChildOfGroup)
                    a._groupParentNodeId != o._groupParentNodeId && (this._removeGroupIfHasOneNode(a._groupParentNodeId, a.id),
...
OrgChart.prototype._attachEventHandlers = function(t) {
    t = this.getSvg();
    this.config.enableTouch || /Mobi/i.test(navigator.userAgent) || /Android/i.test(navigator.userAgent) ? (this._addEvent(t, "touchstart", this._globalMouseDownHandler),
...
OrgChart.prototype._addEvent = function(e, t, r, i) {
    ...
    e.getListenerList[t + i]) || (a = this,
    o = r,
    r = function() {
        if (o)
            return o.apply(a, [this, arguments[0]])
    }
    ,
    e.addEventListener ? "mousewheel" == t ? e.addEventListener(t, n, {
        passive: !1
    }) : e.addEventListener(t, n, !1) : e.attachEvent("on" + t, function() {
        var t = r.call(e, window.event);
        return !1 === t && (window.event.returnValue = !1,
...
OrgChart.MAXIMIZE = '<g style="cursor:pointer;" transform="matrix(1,0,0,1,{x},20)" control-maxmin-id="{id}"><rect x="0" y="0" width="26" height="26" fill="#EEEEEE" stroke="#aeaeae" stroke-width="1" rx="2"></rect><polyline  stroke="#aeaeae" stroke-width="2" points="9,6 17,13 9,20" fill="none"></polyline></g>',
...
OrgChart.input = function(t, e, r, i, a, o) {
    ...
    i || s.addEventListener("focus", function() {
...
OrgChart._intersects = function(t, e, r) {
    ...
    return 0 != (l = (o - i) * (p - c) - (g - d) * (n - a)) && (s = ((a - n) * (g - i) + (o - i) * (p - a)) / l,
    0 < (h = ((p - c) * (g - i) + (d - g) * (p - a)) / l) && h < 1 && 0 < s && s < 1)
}
,
...
OrgChart._addPointTop = function(t, e, r, i, a) {
    var o, n, l;
    return "left" == a ? o = t.leftNeighbor ? t.x + (t.leftNeighbor.x + t.leftNeighbor.w - t.x) / 2 : t.x - r.siblingSeparation / 2 : "right" == a && (o = t.rightNeighbor ? t.x + t.w + (t.rightNeighbor.x - (t.x + t.w)) / 2 : t.x + t.w + r.siblingSeparation / 2),
...
OrgChart._addPointLeft = function(t, e, r, i, a) {
    ...
    return "bottom" == a ? o = t.rightNeighbor ? t.y + t.h + (t.rightNeighbor.y - (t.y + t.h)) / 2 : t.y + t.h + r.siblingSeparation / 2 : "top" == a && (o = t.leftNeighbor ? t.y + (t.leftNeighbor.y + t.leftNeighbor.h - t.y) / 2 : t.y - r.siblingSeparation / 2),
...
OrgChart.editUI.prototype._createDetailsView = function(t, e) {
    ...
    for (var h = 0; h < l.length; h++) {
        var d = s[l[h]];
        if (t.isGroup && (d = this.obj.config.tags[t.id]["group" + l[h]]),
        OrgChart._fieldIsImg(this.obj.config, l[h])) {
            ...
            this._addField(o, OrgChart.input(l[h], d, null, !0, void 0, r), l[h], "details")
    }
    ...
    OrgChart.anim(this.wrapperElement, {
        ...
        o.style.height = r.obj.element.offsetHeight - i.offsetHeight + "px"
    })
}
,
...
OrgChart.editUI.prototype._createEditView = function(t, e) {
    ...
    for (var c = 0; c < h.length; c++) {
        var g = d[h[c]];
        if (t.isGroup && (g = this.obj.config.tags[t.id]["group" + h[c]]),
        "tags" != h[c])
            if (OrgChart._fieldIsImg(this.obj.config, h[c])) {
                ...
                this._addField(n, OrgChart.input(h[c], g, null, !1, void 0, i), h[c], "edit")
    }
    t.isGroup || t.isChildOfGroup || 0 != t.childrenIds.length || this._addField(n, l, "isAssistant", "edit"),
    ...
    o.addEventListener("click", function() {
        if ("Save" == o.innerHTML)
            OrgChart.anim(o, {
                ...
                if (n.removeChild(t),
                e && !OrgChart._arrayContains(i.fields, e)) {
    ...
    OrgChart.anim(this.wrapperElement, {
        ...
        n.style.height = i.obj.element.offsetHeight - r.offsetHeight + "px",
        1 < i.wrapperElement.getElementsByTagName("input").length && i.wrapperElement.getElementsByTagName("input")[0].focus()
    })
}
,
            ...
            for (var r = this.wrapperElement.querySelectorAll("[data-field-name]"), i = 0; i < r.length; i++) {
                ...
                if (a) {
                    ...
                    if (a === OrgChart.TAGS)
                        ...
                        null != e[a] ? e[a] = o : "" != o && (e[a] = o)
                }
            }
            var l = this;
            this.obj._hideBeforeAnimation(),
            OrgChart.anim(l.wrapperElement, {
                right: 0,
                opacity: 1
            }, {
                right: -150,
                ...
                l.node.isGroup ? l.obj._updateTag(l.node.id, "groupName", e.Name) : l.obj.updateNode(e, !0),
...
OrgChart.menuUI.prototype._show = function(t, e, o, n, l) {
    ...
    if (!1 === OrgChart.events.publish("show", [this, i]))
        return !1;
    for (var a in l = i.menu) {
        ...
        r += '<div data-item="' + a + '" style="border-bottom: 1px solid #D7D7D7; padding: 7px 10px;color: #7A7A7A;">' + h + "<span>&nbsp;&nbsp;" + d + "</span></div>"
    }
    if ("" != r) {
            ...
            a.addEventListener("click", function(t) {
                ...
                if (void 0 === l[r].onClick)
                    if ("add" === r) {
                        ...
                        "remove" === r ? s.obj.removeNode(o, !0) : "svg" === r ? s.obj.exportSVG({
...
OrgChart.idb._open = function(e) {
    if ((0 < navigator.userAgent.toLowerCase().indexOf("safari") || 0 < navigator.userAgent.toLowerCase().indexOf("firefox")) && window.location !== window.parent.location)
        e && e(!1);
    else {
        if (!window.indexedDB)
        ...
        if (null == OrgChart.idb.db) {
            ...
            t.onsuccess = function(t) {
                OrgChart.idb.db = t.target.result,
                e && e(!0)
            }
            ,
...
OrgChart.toolbarUI.prototype.init = function(t, e) {
    if (e) {
        ...
        e.zoom && (this.div.innerHTML += '<div data-tlbr="plus">' + OrgChart.toolbarUI.zoomInIcon + "</div>",
        this.div.innerHTML += '<div data-tlbr="minus">' + OrgChart.toolbarUI.zoomOutIcon + "</div>"),
        e.layout && (this.div.innerHTML += '<div data-tlbr="layout">' + OrgChart.toolbarUI.layoutIcon + "</div>",
        ...
        var r = this.div.querySelector('[data-tlbr="plus"]')
          ...
          , o = this.div.querySelector('[data-tlbr="fullScreen"]')
    ...
    this._draw(!1, OrgChart.action.exporting, t, function(t) {
        ...
        "svg" == a.ext)
            if (o)
                ...
                if (!1 === i)
                    return !1;
                if (OrgChart._downloadFile(a.mime, e.content, e.options.filename, e.options.openInNewTab),
            ...
            n._pages(a, r.querySelector("svg"), function(t) {
                ...
                o ? o(n, e, r.querySelector("svg")) : (e = JSON.stringify(e),
                OrgChart._ajax(n.config.exportUrl + "/v3", "POST", e, "arraybuffer", function(t) {
                    if (!1 === OrgChart.events.publish("exportend", [n, t]))
                        return !1;
                    OrgChart.loading.hide(n),
                    OrgChart._downloadFile(a.mime, t, a.filename, a.openInNewTab)
                }))
            })
    })
}
...
OrgChart.prototype._pagesAfit = function(t, e) {
    ...
    var o = t.landscape ? a - (t.margin[1] + t.margin[3]) : i - (t.margin[1] + t.margin[3])
      , n = (t.landscape ? (t.margin[0],
    t.margin[2]) : (t.margin[0],
    t.margin[2]),
    o / r);
    return this._pagesA100(t, e, 100 * n)
}
,
OrgChart.prototype._pagesA100 = function(t, e, r) {
    ...
    for (var g = n, p = l, f = d.w * (100 / r) - n, u = d.h * (100 / r), C = []; g < s - 1; ) {
        for (; p < h - 1; ) {
            var m = [g, p, g + f, u];
            m = m.join(),
            C.push({
            ...
            if (n[t]) {
                ...
                if (0 < r.length) {
                    ...
                    for (i = 0; i < r.length; i++)
                        1 == e.length ? o = r[i](e[0]) && o : 2 == e.length ? o = r[i](e[0], e[1]) && o : 3 == e.length ? o = r[i](e[0], e[1], e[2]) && o : 4 == e.length ? o = r[i](e[0], e[1], e[2], e[3]) && o : 5 == e.length && (o = r[i](e[0], e[1], e[2], e[3], e[4]) && o);
                    return o
                }
            }
        }
...
OrgChart.prototype.importCSV = function() {
    ...
    t.onchange = function(t) {
        ...
        s.onload = function() {
            ...
            0 != OrgChart.events.publish("import", [h, r]) && (h.config.nodes = r,
            h.draw())
        }
        ,
...
OrgChart.prototype.importXML = function() {
    ...
    t.onchange = function(t) {
        ...
        r.onload = function() {
            ...
            0 != OrgChart.events.publish("import", [i, e]) && (i.config.nodes = e,
            i.draw())
        }
        ,
...
OrgChart.prototype._expCollHandler = function(t) {
    ...
    if (e.collapsedChildrenIds.length) {
        if (!1 === OrgChart.events.publish("expcollclick", [this, OrgChart.EXPAND, t, e.collapsedChildrenIds]))
            return !1;
        this.expand(t, e.collapsedChildrenIds, !1)
    } else {
        if (!1 === OrgChart.events.publish("expcollclick", [this, OrgChart.COLLAPSE, t, e.childrenIds]))
            return !1;
...
OrgChart.prototype._maxMinHandler = function(t) {
    ...
    if (e.groupState == OrgChart.EXPAND) {
        if (!1 === OrgChart.events.publish("expcollclick", [this, OrgChart.COLLAPSE, t]))
            return !1;
        this.minimize(t)
    } else if (e.groupState == OrgChart.COLLAPSE) {
        if (!1 === OrgChart.events.publish("expcollclick", [this, OrgChart.EXPAND, t]))
            return !1;
        this.maximize(t)
    }
}
...
OrgChart.prototype._nodeMouseDownHandler = function(t, e, n) {
    if (this.config.enableDragDrop) {
        ...
        if (!1 !== OrgChart.events.publish("drag", [this, i])) {
            ...
            var m = this
              , O = this.getSvg();
            O.classList.add("cursor-grab"),
            O.classList.remove("cursor-move");
            var v = {
                x: r.x,
                y: r.y
            }
              , x = OrgChart._getTransform(t)
              , y = x[4]
              , b = x[5]
              , w = m.getScale()
              , _ = t.cloneNode(!0);
            if (O.insertBefore(_, O.firstChild),
            _.style.opacity = .7,
            l.isChildOfGroup) {
                ...
                x[4] = y,
                x[5] = b,
                _.setAttribute("transform", "matrix(" + x.toString() + ")")
            }
            var k = function(t, e) {
                if (null != t) {
                    var r = m.getBGNode(t);
                    r.isChildOfGroup && (m.getNodeElement(r._groupParentNodeId).style.opacity = 1),
                    e.style.opacity = 1
                }
            }
              , s = function(t) {
                if (v) {
                    var e = OrgChart._getClientXY(t)
                      ...
                      , i = OrgChart._getOffsetXY(m.element, t)
                      , a = {
                        left: m.width() - (i.x + m.config.padding) < 0,
                        right: i.x - m.config.padding < 0,
                        down: m.height() - (i.y + m.config.padding) < 0,
                        up: i.y - m.config.padding < 0
                    };
                    if (a.left || a.right || a.up || a.down) {
                        O.classList.remove("cursor-grab"),
                        O.classList.add("cursor-move");
                        var o = x[4]
                          , n = x[5]
                          , l = v.x
                          , s = v.y
                          , h = e.x
                          , d = e.y;
                        m.startMove(a, function(t) {
                            x[4] = o + t.x,
                            x[5] = n + t.y,
                            v.x = l - t.xWithoutScale,
                            v.y = s - t.yWithoutScale,
                            e.x = h - t.xWithoutScale,
                            e.y = d - t.yWithoutScale,
                            _.setAttribute("transform", "matrix(" + x.toString() + ")")
                        })
                    } else {
                        if (m.stopMove(),
                        ...
                        if (null != u) {
                            var g = m.getBGNode(u);
                            g.isChildOfGroup && (m.getNodeElement(g._groupParentNodeId).style.opacity = .8),
                            C.style.opacity = .5
                        }
                        var p = (e.x - v.x) / w
                          , f = (e.y - v.y) / w;
              ...
              , h = function(t) {
                ...
                O.classList.remove("cursor-grab"),
                O.classList.remove("cursor-move"),
                O.removeEventListener(n.move, s),
                O.removeEventListener(n.up, h),
                n.leave && O.removeEventListener(n.leave, h),
                l.id == u || l.isChildOfGroup && l._groupParentNodeId == u || null == u)
                    return k(u, C),
                    O.removeChild(_),
                    void (m._gragStartedId = null);
                var e = m.getBGNode(u);
                if (e.isChildOfGroup && l.isChildOfGroup && e._groupParentNodeId == l._groupParentNodeId)
                    return k(u, C),
                    O.removeChild(_),
                    void (m._gragStartedId = null);
                if (!1 === OrgChart.events.publish("drop", [m, l.id, e.id]))
                    return k(u, C),
                    O.removeChild(_),
                    void (m._gragStartedId = null);
                if (null == m.config.dragDropMenu || l.isGroup && !e.isGroup)
                    m._canUpdateLink(l.id, u) ? m.linkNode(l.id, u) : (O.removeChild(_),
                    k(u, C));
                else if (m._canUpdateLink(l.id, u)) {
                    var r = OrgChart._getOffsetXY(m.element, t)
                      , i = null;
                    if (Array.isArray(l.tags))
                        for (var a = 0; a < l.tags.length; a++) {
                            var o = l.tags[a];
                            m.config.tags[o] && m.config.tags[o].dragDropMenu && (i = m.config.tags[o].dragDropMenu)
                        }
                    m.dragDropMenuUI.show(r.x, r.y, l.id, u, i)
                } else
                    O.removeChild(_),
                    k(u, C);
                m._gragStartedId = null
            };
            O.addEventListener(n.move, s),
            O.addEventListener(n.up, h),
            n.leave && O.addEventListener(n.leave, h)
        }
    }
}
,
...
OrgChart.prototype._commonClickHandler = function(t, e, r) {
    ...
    if (r == OrgChart.action.expandCollapse) {
        if (i.collapsedChildrenIds.length) {
            if (!1 === OrgChart.events.publish("expcollclick", [this, OrgChart.EXPAND, t, i.collapsedChildrenIds]))
                return !1;
            this.expand(t, i.collapsedChildrenIds, !1)
        } else {
            if (!1 === OrgChart.events.publish("expcollclick", [this, OrgChart.COLLAPSE, t, i.childrenIds]))
                return !1;
            this.collapse(t, i.childrenIds, !1)
        }
        this.ripple(i.id, e.clientX, e.clientY)
    }
    r == OrgChart.action.edit && (this.editUI.show(i.id),
    this.ripple(i.id, e.clientX, e.clientY)),
    r == OrgChart.action.details && (this.editUI.show(i.id, !0),
    this.ripple(i.id, e.clientX, e.clientY))
}
,
    ...
    if (t) {
        ...
        !this._moveInterval) {
            ...
            this._moveInterval = setInterval(function() {
                ...
                r._movePosition.left && (o++,
                t.x = o * OrgChart.MOVE_STEP / a,
                t.xWithoutScale = o * OrgChart.MOVE_STEP),
                r._movePosition.right && (o++,
                t.x = -o * OrgChart.MOVE_STEP / a,
                t.xWithoutScale = -o * OrgChart.MOVE_STEP),
                r._movePosition.up && (n++,
                t.y = -n * OrgChart.MOVE_STEP / a,
                t.yWithoutScale = -n * OrgChart.MOVE_STEP),
                r._movePosition.down && (n++,
                t.y = n * OrgChart.MOVE_STEP / a,
                t.yWithoutScale = n * OrgChart.MOVE_STEP),
...
OrgChart.prototype._mouseDownHandler = function(t, e, r) {
    ...
    e.touches && 1 < e.touches.length && (c.type = "pinch",
    c.dist = Math.sqrt((i.x - a.x) * (i.x - a.x) + (i.y - a.y) * (i.y - a.y)));
    ...
    var p = function(t) {
      ...
      , f = function() {
        ...
        }) : "pan" != c.type || s.config.sticky || s._draw(!0, OrgChart.action.pan),
        c = null,
...
OrgChart.remote._toReqDTO = function(t, e, r) {
    ...
    0 < t.children.length && (i.c = OrgChart.remote._convertToIdArray(t.children)),
...
OrgChart.remote._walk = function(r, t, i) {
    for (var e = [], a = [], o = [t.orientation, t.levelSeparation, t.mixedHierarchyNodesSeparation, t.subtreeSeparation, t.siblingSeparation, t.template], n = 0; n < r.length; n++)
        a.push(r[n].id),
        OrgChart.remote._toReqDTO(r[n], e, t.template);
    var l = {
        n: e,
        c: o,
        r: a,
    ...
    null != OrgChart.remote._fromReqDTO ? OrgChart.remote._fromReqDTO(l.n, l.r, l.c, function(t) {
    ...
    OrgChart.idb.read(l, function(t, e) {
        t ? OrgChart.remote._proceed(r, e.res, i) : OrgChart.remote._findRegion(function(t) {
            OrgChart._ajax(t, "post", l, "json", function(t) {
                t.error ? i(2) : (OrgChart.remote._proceed(r, t, i),
                OrgChart.idb.write({
                    id: l,
                    res: t
                }))
...
OrgChart.remote._findRegion = function(r) {
    ...
    if (t)
        ...
        for (var i = ["au-e", "au-se", "brs", "ca", "ca-e", "easia", "eus-2", "eus", "fr", "ind", "jp-e", "jp-w", "kr", "n-eu", "se-asia", "s-ind", "uk-s", "uk-w", "us", "us-n-c", "us-s-c", "w-c-us", "w-eu", "w-ind", "w-us-2", "wus"], a = [], o = 0; o < i.length; o++)
            ...
            !function() {
                ...
                t.onreadystatechange = function() {
                    if (4 == this.readyState && 200 == this.status) {
                        OrgChart.remote.setItem("funcUrl", e),
                        r(e);
...
OrgChart.searchUI.prototype.hide = function() {
    ...
    if (t) {
        ...
        r.value = "",
        i.innerHTML = "",
        "none" != e.style.display && "none" != t.style.display && OrgChart.anim(e, {
...
OrgChart.searchUI.prototype.addSearchControl = function() {
    ...
    t.innerHTML += OrgChart.searchUI.createInputField(this.obj.config.padding),
    ...
    r.addEventListener("keyup", function(t) {
        40 == t.keyCode ? o() : 38 == t.keyCode ? n() : 13 == t.keyCode ? l() : 27 == t.keyCode ? i.hide() : i._serverSearch(this.value)
    });
    var o = function() {
        var t = a.querySelectorAll("[data-search-item-id]")
          , e = a.querySelector('[data-selected="yes"]');
        null == e && 0 < t.length ? (t[0].setAttribute("data-selected", "yes"),
        t[0].style.backgroundColor = "#F0F0F0") : 0 < t.length && e.nextSibling && (e.setAttribute("data-selected", "no"),
      ...
      , n = function() {
        var t = a.querySelectorAll("[data-search-item-id]")
          , e = a.querySelector('[data-selected="yes"]');
        null == e && 0 < t.length ? (t[t.length - 1].setAttribute("data-selected", "yes"),
        t[t.length - 1].style.backgroundColor = "#F0F0F0") : 0 < t.length && e.previousSibling && (e.setAttribute("data-selected", "no"),
        e.style.backgroundColor = "inherit",
...
OrgChart.searchUI.prototype._serverSearch = function(t) {
    for (var e = this, r = this.obj.element.querySelector('[data-id="container"]'), i = this.obj.element.querySelector('[data-id="search"]'), a = this.obj.server.find(t), o = "", n = 0; n < a.length; n++) {
        var l = a[n]
          , s = "";
        l.img && (s = '<img style="padding: 2px 0px  2px 7px;width:32px;height:32px;" src="' + l.img + '" / >'),
...
OrgChart.server.prototype.read = function(t, r, i, e, a, o, n, l) {
    ...
    if (t) {
        ...
        a != OrgChart.action.exporting && (this.maxX = h,
        this.maxY = d,
        this.minX = c,
        this.minY = g,
        this.root = p,
        ...
        this._read(function(t) {
            ...
            e.notif = t.limit,
            a != OrgChart.action.exporting && (s.maxX = h,
            s.maxY = d,
            s.minX = c,
            s.minY = g,
            s.root = p,
    ...
    for (s = 0; s < n.length && s != OrgChart.SEARCH_RESULT_LIMIT; s++) {
        ...
        if (!g) {
            ...
            for (p = 0; p < O.length; p++) {
                d = O[p],
                c = n[s].data[d];
                d === n[s].propertyName && (e = c,
                i = t,
                void 0,
                a = OrgChart.accentFold(e).indexOf(i),
                c = e.toString().splice(a + i.length, 0, "</mark>").splice(a, 0, "<mark>")),
                d != OrgChart.TAGS && (d == OrgChart.ID ? u = u + c + ", " : OrgChart._fieldIsImg(this.config, d) ? m = c : o(d) && !OrgChart._fieldIsImg(this.config, d) ? f = f + c + ", " : OrgChart._fieldIsImg(this.config, d) || (C = C + c + ", "))
            }
...
OrgChart.server._createNodes = function(a, t, e, r) {
    for (var i = a.nodes, o = {}, n = [], l = [], s = [], h = {}, d = !1, c = 0; c < i.length; c++) {
        ...
        if ((v = new OrgChart.node(g.id,g.pid,p,u)).isAssistant = -1 != p.indexOf("assistant"),
        v.isAssistant && -1 == s.indexOf(v.pid) && OrgChart.server._isNotNullEmptyUndefined(v.pid) && s.push(v.pid),
        null != f) {
            if (!o[f.name]) {
                n.push(f.name);
                var C = new OrgChart.node(f.name,g.pid,[f.name],f.tag.template);
                C.isGroup = !0,
                null != a.orderBy && (C.order = OrgChart.server._getOrderFieldValue(g, a.orderBy)),
                o[f.name] = C
            }
            v.isChildOfGroup = !0,
            ...
            null != a.orderBy && (v.order = OrgChart.server._getOrderFieldValue(g, a.orderBy)),
            o[g.id] = v,
            n.push(g.id)
    }
    if (null != a.orderBy && n.sort(function(t, e) {
        var r = o[t].order
          , i = o[e].order;
        return "number" == typeof r || "number" == typeof i ? (null == r && (r = -1),
        null == i && (i = -1),
        a.orderBy.desc ? i - r : r - i) : "string" == typeof r || "string" == typeof i ? (null == r && (r = ""),
        null == i && (i = ""),
        a.orderBy.desc ? i.localeCompare(r) : r.localeCompare(i)) : void 0
    }),
    d)
    ...
    for (c = 0; c < n.length; c++) {
        ...
        if (OrgChart.server._isNotNullEmptyUndefined(v.pid))
            (H = o[v.pid]) ? ((v.parent = H).children.push(v),
            H.childrenIds.push(v.id)) : (v.pid = null,
            v.parent = null,
            v.isAssistant = !1,
            l.push(v));
        else
        ...
        OrgChart.server._setNodeGroupState(v, x.groupState, a.orientation)) : OrgChart.server._setNodeGroupState(v, OrgChart.EXPAND, a.orientation),
    ...
    if (null != a.roots) {
        ...
        for (c = 0; c < a.roots.length; c++) {
            ...
            if (y) {
                if (y.parent)
                    for (var b = (H = y.parent).children.length - 1; 0 <= b; b--) {
                        (U = H.children[b]) == y && (U.parent = null,
    ...
    for (c = 0; c < s.length; c++) {
        ...
        if (H) {
            ...
            (x = r[_.id]) && (o[_.id].state = x.state,
            o[_.id].state == OrgChart.COLLAPSE && m.push(_.id)),
            OrgChart.server._initStates(_, o, t, a, e, m);
            var k = [];
            for (b = H.children.length - 1; 0 <= b; b--) {
                (U = H.children[b]).isAssistant ? (U.parent = null,
                H.children.splice(b, 1),
                H.childrenIds.splice(b, 1),
                k.splice(0, 0, U.id)) : (U.pid = _.id,
                (U.parent = _).children.unshift(U),
                _.childrenIds.unshift(U.id),
                H.children.splice(b, 1))
            }
            if (k.length % 2) {
                var S = o[k[k.length - 1]]
                  , I = [S.w, S.h]
                  , A = "transperant_" + S.templateName;
                OrgChart.templates[A] = {
                    linkAdjuster: {},
                    link: "",
                    node: "",
                    nodeMenuButton: "",
                ...
                ($ = new OrgChart.node(H.id + "_transperant",S.pid,[],A)).isAssistant = !0,
                (x = r[(o[$.id] = $).id]) && (o[$.id].state = x.state,
                o[$.id].state == OrgChart.COLLAPSE && -1 == m.indexOf($.id) && m.push($.id)),
                k.splice(k.length - 1, 0, $.id),
                OrgChart.server._initStates($, o, t, a, e, m)
            }
            var N = 1;
            for (b = k.length - 1; 0 <= b; b--) {
                if (b % 2 && b != k.length - 1)
                    (Q = new OrgChart.node(H.id + "_split_" + N,null,[],"split")).isAssistant = !0,
                    o[Q.id] = Q,
                    k.splice(b, 0, Q.id),
                    N++;
                else
                    b % 2 && k.splice(b, 0, _.id)
            }
            for (b = 0; b < k.length; b += 3) {
                ...
                E.children.push(L),
                E.children.push(B),
                E.childrenIds.push(M.id),
                E.childrenIds.push(L.id),
                E.childrenIds.push(B.id);
                for (var T = E; "split" == T.templateName && (T = T.parent); )
                    T.childrenIds.push(M.id),
                    T.childrenIds.push(L.id),
                    T.childrenIds.push(B.id);
                (x = r[L.id]) && (o[L.id].state = x.state,
                o[L.id].state == OrgChart.COLLAPSE && -1 == m.indexOf(L.id) && m.push(L.id)),
                OrgChart.server._initStates(L, o, t, a, e, m)
            }
        }
    }
    if (t == OrgChart.action.centerNode) {
        ...
        -1 != (V = m.indexOf(P.id)) && m.splice(V, 1);
        for (var H = OrgChart.server._getRealParent(P), F = P.id; null != H; ) {
            if (-1 != (V = m.indexOf(H.id)) && m.splice(V, 1),
            H.state = OrgChart.EXPAND,
            e.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS)
                for (c = 0; c < H.childrenIds.length; c++) {
                    var U = o[H.childrenIds[c]];
                    if (!OrgChart.server._isSplit(U) && !U.isAssistant && U.id != F)
                        -1 == (V = m.indexOf(H.childrenIds[c])) && m.push(H.childrenIds[c]),
                        U.state = OrgChart.COLLAPSE
                }
            if (H.state == OrgChart.EXPAND)
                for (var z = 0; z < H.childrenIds.length; z++) {
                    var j = o[H.childrenIds[z]];
                    if (OrgChart.server._isSplit(j) || j.isAssistant)
                        -1 != (V = m.indexOf(j.id)) && m.splice(V, 1),
                        j.state = OrgChart.EXPAND
                }
            F = H.id,
            H = OrgChart.server._getRealParent(H)
        }
        if (e.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS)
            for (c = 0; c < P.childrenIds.length; c++) {
                if (-1 != (V = m.indexOf(P.childrenIds[c])) && m.splice(V, 1),
                (U = o[P.childrenIds[c]]).state = OrgChart.EXPAND,
                !U.isAssistant && !OrgChart.server._isSplit(U))
                    for (b = 0; b < U.childrenIds.length; b++) {
                        var D = o[U.childrenIds[b]];
                        -1 == (V = m.indexOf(U.childrenIds[b])) && m.push(U.childrenIds[b]),
                        D.state = OrgChart.COLLAPSE
                    }
            }
    }
    ...
    for (c = 0; c < m.length; c++) {
        if (H = (v = o[m[c]]).parent)
            for (b = H.children.length - 1; 0 <= b; b--) {
                if (H.children[b] == v)
                    if (-1 == H.collapsedChildrenIds.indexOf(v.id) && H.collapsedChildrenIds.push(v.id),
                    H.children.splice(b, 1),
                    "split" == H.templateName) {
                        if (E = v.parent) {
                            for (; "split" == E.templateName; )
                                E = E.parent;
                            for (var G = E.childrenIds.length - 1; 0 <= G; G--) {
                                var R = E.childrenIds[G];
                                -1 == E.collapsedChildrenIds.indexOf(R) && E.collapsedChildrenIds.push(R)
                            }
                        }
                    } else
                        v.parent = null
    ...
    if (OrgChart.server._hasSubLevels(a)) {
        var X = [];
        for (c = 0; c < n.length; c++) {
            v = o[n[c]];
            var Y = OrgChart._getSubLevelsTag(v.tags, a.tags);
            if (Y)
                for (var q = 0; q < Y.tag.subLevels; q++) {
                    var V, W = new OrgChart.node(v.id + "_sub_level_index_" + q,v.pid,[],"subLevel");
                    if (H = o[v.pid])
                        -1 < (V = H.childrenIds.indexOf(v.id)) && H.childrenIds.splice(V, 0, W.id),
                        -1 < (V = H.collapsedChildrenIds.indexOf(W.id)) && H.collapsedChildrenIds.push(v.id),
                        -1 < (V = H.children.indexOf(v)) && (H.children.splice(V, 1),
                        H.children.splice(V, 0, W)),
                        W.children.push(v),
                        W.childrenIds.push(v.id),
                        W.parent = H,
                        v.parent = W,
                        v.pid = W.id,
                        o[W.id] = W,
            ...
            if (0 == (v = o[n[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)
                if (H = v.parent) {
                    if (!OrgChart.server._mixedLayoutAllNodes(H))
                        continue;
                    for (b = H.children.length - 1; 0 <= b; b--) {
                        if ((U = H.children[b]) != v && 0 == U.childrenIds.length && !U.isAssistant && "split" != U.templateName) {
                            U.parent = v,
                            U.pid = v.id,
                            U.layout = OrgChart.mixed,
                            H.children.splice(b, 1);
                            var J = v.children.length;
                            v.children[J] = U,
                            v.layout = OrgChart.mixed
                        }
                    }
                }
        }
    if (a.layout == OrgChart.tree || a.layout == OrgChart.treeRightOffset || a.layout == OrgChart.treeLeftOffset) {
        ...
        for (c = 0; c < n.length; c++) {
            if (0 == (v = o[n[c]]).childrenIds.length && !v.isAssistant && "split" != v.templateName)
                (H = v.parent) && -1 == Z.indexOf(H.id) && Z.push(H.id)
        }
        for (c = 0; c < Z.length; c++) {
            ...
            for (b = H.children.length - 1; 0 <= b; b--) {
                if (OrgChart.server._mixedLayoutAllNodes(H))
                    if (0 == (U = H.children[b]).childrenIds.length && !U.isAssistant && "split" != U.templateName && (U.parent && "split" != U.parent.templateName || U.parent.isAssistant)) {
                        if ((U = H.children[b]).parent = null,
                        U.layout = OrgChart.tree,
                        H.children.splice(b, 1),
                        H.childrenIds.splice(b, 1),
                        a.layout == OrgChart.treeRightOffset && K.splice(0, 0, U.id),
                        a.layout == OrgChart.treeLeftOffset || a.layout == OrgChart.treeRightOffset) {
                            var $;
                            A = "transperant_" + U.templateName;
                            OrgChart.templates[A] = {
                                linkAdjuster: {},
                                link: "",
                                node: "",
                            ...
                            ($ = new OrgChart.node(U.id + "_transperant",null,[],A)).layout = OrgChart.tree,
                            o[$.id] = $,
                            K.splice(0, 0, $.id)
                        }
                        a.layout != OrgChart.treeRightOffset && K.splice(0, 0, U.id)
            ...
            b = K.length - 1; 0 <= b; b--) {
                var Q;
                if (b % 2 && b != K.length - 1)
                    (Q = new OrgChart.node(H.id + "_split_" + N,null,[],"split")).layout = OrgChart.tree,
                    o[Q.id] = Q,
                    K.splice(b, 0, Q.id),
                    N++;
                else
...
OrgChart.server._mixedLayoutAllNodes = function(t) {
    ...
    for (var e = t.children.length - 1; 0 <= e; e--) {
        var r = t.children[e];
        if (0 < r.childrenIds.length || r.isAssistant || "split" == r.templateName)
            return !1
    }
    return !0
}
...
OrgChart.server._getRealParent = function(t) {
    ...
    if (!t.parent)
        return null;
    if ("split" != t.parent.templateName)
        return t.parent;
    for (var e = t.parent; e && "split" == e.templateName; )
        e = e.parent;
    return e
}
,
    ...
    if (r == OrgChart.action.init && t) {
        if (t.tags)
            for (var n = 0; n < t.tags.length; n++) {
                var l = t.tags[n];
                i.tags && i.tags[l] && null != i.tags[l].state && (i.tags[l].state == OrgChart.COLLAPSE && o.push(t.id),
...
OrgChart.server._expandCollapseToLevel = function(t, e, r, i, a, o) {
    if (i.collapse.level) {
        ...
        if (!i.roots || -1 == i.roots.indexOf(t.id)) {
            for (var l = e[t.pid]; OrgChart.server._isSplit(l); )
                l = e[l.pid];
        ...
        if (n >= i.collapse.level) {
            ...
            if (Array.isArray(i.expand.nodes))
                if (i.expand.allChildren) {
                    ...
                    s = -1 != i.expand.nodes.indexOf(t.id);
            !s && i.collapse.allChildren ? (o.push(t.id),
            t.state = OrgChart.COLLAPSE) : s || i.collapse.allChildren || n != i.collapse.level || (o.push(t.id),
            t.state = OrgChart.COLLAPSE)
        }
    }
}
,
...
OrgChart.server._initNodeSize = function(t, e, r) {
    ...
    if (t.isGroup && t.groupState == OrgChart.EXPAND) {
        ...
        case OrgChart.orientation.bottom_left:
            ...
            a = h * l + i.groupNodesSeparation * l + i.groupNodesSeparation + i.groupPadding[1] + i.groupPadding[3],
            o = d * s + i.groupNodesSeparation * s + i.groupNodesSeparation + i.groupPadding[0] + i.groupPadding[2];
            break;
        case OrgChart.orientation.right:
        case OrgChart.orientation.left:
        case OrgChart.orientation.right_top:
...
OrgChart.server._setNodeGroupState = function(t, e, r) {
    t.groupState = e,
    OrgChart.server._initNodeSize(t, t.templateName, r)
}
,
...
OrgChart.server.getAllFields = function(t) {
    ...
    for (r = 0; r < t.nodes.length; r++)
        for (var i in t.nodes[r])
            i !== OrgChart.ID && i !== OrgChart.TAGS && i !== OrgChart.NODES && i !== OrgChart.PID && (t.nodeBinding[i] || OrgChart._arrayContains(e, i) || e.push(i));
    return e
}
,
...
OrgChart.server._setGroupNodes = function(t, e) {
    if (t.isGroup && t.groupState == OrgChart.EXPAND)
        for (var r = OrgChart.t(t.templateName), i = 0, a = 0, o = 0; o < t.bgnodes.length; o++) {
            var n = t.bgnodes[o];
            n.x = t._groupSettings._cellWidth / 2 - n.w / 2 + r.groupNodesSeparation + (t._groupSettings._cellWidth * i + r.groupNodesSeparation * i),
            n.y = t._groupSettings._cellHeight / 2 - n.h / 2 + r.groupNodesSeparation + (t._groupSettings._cellHeight * a + r.groupNodesSeparation * a),
            i >= t._groupSettings._columns - 1 ? (i = 0,
            a++) : i++
        }
}
,
OrgChart.server._getResponse = function(t, e, r, i, a, o, n, l, s, h, d, c, g, p, f) {
    ...
    null == s && i.align == OrgChart.CENTER) {
        ...
        if (y - 2 * i.padding >= a - n)
            ...
            switch (w = h.x - y / 2 + OrgChart.server._getNodeWidth(h, i) / 2,
            i.orientation) {
            case OrgChart.orientation.right:
            case OrgChart.orientation.right_top:
        ...
        if (b - 2 * i.padding >= o - l)
            ...
            switch ((_ = -(b / 2 - (o - l) / 2)) > -i.padding && (_ = -i.padding),
            i.orientation) {
            case OrgChart.orientation.bottom:
            case OrgChart.orientation.bottom_left:
        ...
        case OrgChart.orientation.top:
            w = h.x - y / 2 + OrgChart.server._getNodeWidth(h, i) / 2,
            _ = -i.padding;
            break;
        case OrgChart.orientation.bottom:
            w = h.x - y / 2 + OrgChart.server._getNodeWidth(h, i) / 2,
    ...
    d == OrgChart.action.centerNode && ((B = g[c.id]) || (B = g[p[c.id]]),
    ...
    if (d == OrgChart.action.insert || d == OrgChart.action.expandCollapse || d == OrgChart.action.update || d == OrgChart.action.centerNode) {
        var k = null;
        if (d == OrgChart.action.insert && c && null != c.insertedNodeId && null != c.insertedNodeId)
            (k = g[c.insertedNodeId]) || (k = g[p[c.insertedNodeId]]);
        else if (d == OrgChart.action.update && c && null != c.visId && null != c.visId)
            (k = g[c.visId]) || (k = g[p[c.visId]]);
        else if (d == OrgChart.action.expandCollapse && c && null != c.id && null != c.id)
            (k = g[c.id]) || (k = g[p[c.id]]),
    ...
    for (var I in g) {
        ...
        if (OrgChart.server._isVisible(A, i, s, d)) {
            u[A.id] = A;
            var N = null;
            if (d == OrgChart.action.expandCollapse && f && f[A.id] && "expandCollapseToLevel" == c.method) {
                if (N = {
                    x: (T = f[A.id]).x,
                    y: T.y
                },
                T) {
                    ...
                    for (var E = A, M = null; null != E; )
                        f[E.id] && f[E.id].state == OrgChart.COLLAPSE && (M = E),
                        E = E.parent;
                    M && M.parent && (N = {
                        x: M.parent.x,
                        y: M.parent.y
            ...
            } else if (d == OrgChart.action.expandCollapse && f && f[A.id]) {
                var L;
                if (N = {
                    x: (T = f[A.id]).x,
                    y: T.y
                ...
                if (L = g[c.id]) {
                    for (E = A.parent; null != E && -1 == c.ids.indexOf(A.id) && -1 == c.ids.indexOf(E.id); )
                        E = E.parent;
                    E && (N = {
                        x: L.x + L.w / 2 - A.w / 2,
                        y: L.y + L.h / 2 - A.h / 2
                ...
                var B;
                if (null != (T = f[A.id]).x && null != T.y && (N = {
                    x: T.x,
                    y: T.y
                }),
                (B = g[c.id]) && B == A)
                    (E = A.parent) && E.id == c.id && (N = {
                        x: B.x + B.w / 2 - A.w / 2,
                        y: B.y + B.h / 2 - A.h / 2
                    })
            ...
            } else if (d == OrgChart.action.insert && c && c.insertedNodeId == A.id)
                N = {
                    x: A.parent.x,
                    y: A.parent.y
                };
                ...
                d !== OrgChart.action.exporting && r && !r[A.id] && (m[0].push(A.id),
                m[1].push({
                    opacity: 0
                }),
                m[2].push({
            ...
            null != N && null != N.x && null != N.y && (N.x == A.x && N.y == A.y || (m[0].push(A.id),
            m[1].push({
...
,
OrgChart.server._hasSubLevels = function(t) {
    if (t.tags)
        for (var e in t.tags)
            if (t.tags[e].subLevels)
                return !0;
    return !1
}
,
...
OrgChart.templates.deborah.nodeMenuButton = '<g style="cursor:pointer;" transform="matrix(1,0,0,1,125,130)" control-node-menu-id="{id}"><rect x="-4" y="-10" fill="#000000" fill-opacity="0" width="22" height="22"></rect><circle cx="0" cy="0" r="2" fill="#ffffff"></circle><circle cx="7" cy="0" r="2" fill="#ffffff"></circle><circle cx="14" cy="0" r="2" fill="#ffffff"></circle></g>',
...
OrgChart.ui = {
    ...
    node: function(t, e, r, i, a, o, n, l, s, h) {
        ...
        if (d.defs && (c = c.replace("{randId}", OrgChart.ui._defsIds[t.templateName])),
        ...
        t.isGroup) {
            ...
            if (g) {
                ...
                c += p.replace("{val}", g).replaceAll("{randId}", OrgChart._randomId()).replaceAll("{randId2}", OrgChart._randomId())
            }
        } else
            for (var f in n) {
                var u = n[f];
                if (e) {
                    var C = e[u];
                    if ("function" == typeof u && (C = u(h, t)),
                    null != C && null != C && null != d[f])
                        c += (OrgChart._lblIsImg(i, f) || "string" != typeof C || (C = OrgChart.wrapText(C, d[f])),
                        d[f].replace("{val}", C)).replaceAll("{randId}", OrgChart._randomId()).replaceAll("{randId2}", OrgChart._randomId())
                }
            }
        ...
        v = v.replace("{opacity}", x),
        t.isGroup || null == i.nodeMenu || "split" == t.templateName || l === OrgChart.action.exporting || (c += d.nodeMenuButton.replace("{id}", t.id));
        var y = OrgChart.ui._renderGroupedNodes(t, d, i, r, n, a, o, l, s, h);
        return t.isGroup && l != OrgChart.action.exporting && (t.groupState == OrgChart.EXPAND ? c += OrgChart.MINIMIZE.replace("{x}", t.w - 45).replace("{id}", t.id) : c += OrgChart.MAXIMIZE.replace("{x}", t.w - 45).replace("{id}", t.id)),
        c = v + c + y + OrgChart.grCloseTag
    },
    ...
    expandCollapse: function(t, e, r, i, a) {
        if (i === OrgChart.action.exporting)
            return "";
        if (0 == e.childrenIds.length)
            return "";
        if ("split" == e.templateName)
            return "";
        var o = ""
          , n = 0
          , l = 0
          , s = OrgChart.t(e.templateName, a);
        switch (r.orientation) {
        case OrgChart.orientation.top:
        case OrgChart.orientation.top_left:
            n = e.x + e.w / 2,
        ...
        if (n -= s.expandCollapseSize / 2,
        l -= s.expandCollapseSize / 2,
        e.collapsedChildrenIds.length ? (o += OrgChart.expcollOpenTag.replace("{id}", e.id).replace("{x}", n).replace("{y}", l),
        o += s.plus) : (o += OrgChart.expcollOpenTag.replace("{id}", e.id).replace("{x}", n).replace("{y}", l),
        o += s.minus),
        (o += OrgChart.grCloseTag).indexOf("{collapsed-children-count}")) {
            var h = OrgChart.collapsedChildrenCount(t, e);
            o = o.replace("{collapsed-children-count}", h)
        }
        return o
    },
    link: function(t, e, r, i) {
        ...
        for (var l = 0; l < t.children.length; l++) {
            ...
            if ((s.isAssistant || 2 == s.layout) && s.rightNeighbor && "split" == s.rightNeighbor.templateName)
                switch (r.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
            ...
            else if ((s.isAssistant || 2 == s.layout) && s.leftNeighbor && "split" == s.leftNeighbor.templateName)
                switch (r.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
            ...
            d = d.replaceAll("{xa}", h.xa).replaceAll("{ya}", h.ya).replaceAll("{xb}", h.xb).replaceAll("{yb}", h.yb).replaceAll("{xc}", h.xc).replaceAll("{yc}", h.yc).replaceAll("{xd}", h.xd).replaceAll("{yd}", h.yd),
            o.push(OrgChart.linkOpenTag.replace("{id}", t.id).replace("{class}", "link " + s.tags.join(" ")).replace("{level}", t.level).replace("{child-id}", s.id)),
            o.push(d);
            var c = "";
            ...
            "" != c && (c = OrgChart.linkFieldsOpenTag.replace("{x}", h.x).replace("{y}", h.y).replace("{rotate}", 0) + c + OrgChart.grCloseTag,
            o.push(c)),
    ...
    _linkTopToBottom: function(t, e, r, i) {
        ...
        return a = t.x + t.w / 2 + r.linkAdjuster.toX,
    ...
    _linkRightToLeft: function(t, e, r, i) {
        var a, o, n, l, s, h, d, c = 0;
        return a = t.x + r.linkAdjuster.toX,
    ...
    _linkLeftToRight: function(t, e, r, i) {
        var a, o, n, l, s, h, d, c = 0;
        return a = t.x + t.w + r.linkAdjuster.toX,
        o = t.y + t.h / 2 + r.linkAdjuster.toY,
...
OrgChart.htmlRipple = function(n) {
    ...
    n.addEventListener("mousedown", function(t) {
        ...
        a = "top:" + (t.pageY - e.top - i / 2) + "px; left: " + o + "px; height: " + i + "px; width: " + i + "px;",
...
OrgChart._menuPosition = function(t, e, r) {
    var i = t.getBoundingClientRect()
      ...
      , n = i.left - a.left
      , l = i.top - a.top;
    return i.top + o.height > a.top + a.height && (l -= o.height),
...
,
OrgChart._getSubLevelsTag = function(t, e) {
    if (t && t.length)
        for (var r = 0; r < t.length; r++) {
            var i = e[t[r]];
            if (i && i.subLevels)
...
OrgChart.wrapText = function(t, e) {
    ...
    if ("multiline" == s) {
        ...
        for (var O = 1, v = 1; v < f.length; v++) {
            var x = C.firstChild.data.length;
            if (C.firstChild.data += " " + f[v],
            C.getComputedTextLength() > l) {
                if (C.firstChild.data = C.firstChild.data.slice(0, x),
                O++,
                0 !== c && c < O)
                    break;
                (C = document.createElementNS(h, "tspan")).setAttributeNS(null, "x", a),
...
OrgChart._getPosition = function(t, e, r, i) {
    ...
    t && 3 == t.length) {
        ...
        -1 != o && null != t[1][o].transform && (null == r && (a.x = t[1][o].transform[4]),
    ...
    for (var l = 0; l < n; l++)
        !function() {
            ...
            OrgChart._getDataUri(r.getAttribute(i), function(t) {
                t.success && r.setAttribute(i, t.result),
                e == n - 1 && a()
            })
        }()
}
...
OrgChart._json2csv = function(t) {
    for (var o = [], e = function(t) {
        ...
        for (var a in t[i])
            OrgChart._arrayContains(o, a) || (o.push(a),
            r += a + ",");
    r += "\n";
...
OrgChart.accentFold = function(t) {
    return (t = t.toString().toLowerCase()).replace(/([Ã Ã¡Ã¢Ã£Ã¤Ã¥])|([Ã§])|([Ã¨Ã©ÃªÃ«])|([Ã¬Ã­Ã®Ã¯])|([Ã±])|([Ã²Ã³Ã´ÃµÃ¶Ã¸])|([ÃŸ])|([Ã¹ÃºÃ»Ã¼])|([Ã¿])|([Ã¦])/g, function(t, e, r, i, a, o, n, l, s, h, d) {
...
OrgChart.childrenCount = function(t, e, r) {
    ...
    for (var a = 0; a < i.length; a++) {
        var o = i[a];
        "string" == typeof o && -1 == o.indexOf("split") && -1 == o.indexOf("transperant") ? -1 == r.indexOf(o) && r.push(o) : "string" != typeof o && -1 == r.indexOf(o) && r.push(o),
        OrgChart.collapsedChildrenCount(t, t.nodes[o], r)
    }
    return r.length
}
,
OrgChart.collapsedChildrenCount = function(t, e, r) {
    var i = e.childrenIds;
    null == r && (r = [],
    i = e.collapsedChildrenIds);
    for (var a = 0; a < i.length; a++) {
        var o = i[a];
        "string" == typeof o && -1 == o.indexOf("split") && -1 == o.indexOf("_sub_level_index_") && -1 == o.indexOf("transperant") ? -1 == r.indexOf(o) && r.push(o) : "string" != typeof o && -1 == r.indexOf(o) && r.push(o),
        OrgChart.collapsedChildrenCount(t, t.nodes[o], r)
    }
    return r.length
}
        ...
        !function(t, i, e) {
            ...
            t.addEventListener("wheel", function(t) {
                ...
                if (n.config.mouseScrool == OrgChart.action.xScroll)
                    (e = t.deltaX || t.wheelDeltaX) || (e = t.deltaY || t.wheelDeltaY);
                else if (n.config.mouseScrool == OrgChart.action.scroll && !(e = t.shiftKey ? t.deltaY || t.wheelDeltaY : t.deltaX || t.wheelDeltaX))
                    return;
                e = -e,
                t.preventDefault(),
        ...
        !function(t, i, e) {
            ...
            t.addEventListener("wheel", function(t) {
                ...
                if (n.config.mouseScrool == OrgChart.action.yScroll)
                    (e = t.deltaY || t.wheelDeltaY) || (e = t.deltaX || t.wheelDeltaX);
                else if (n.config.mouseScrool == OrgChart.action.scroll && !(e = t.shiftKey ? t.deltaX || t.wheelDeltaX : t.deltaY || t.wheelDeltaY))
                    return;
                e = -e,
                t.preventDefault(),
                e = Math.max(-1, Math.min(1, e)),
...
OrgChart.prototype.zoom = function(t, e, r) {
    var i = this.getViewBox().slice(0)
      ...
      , o = i[2]
      , n = i[3];
    !0 === t ? (i[2] = i[2] / OrgChart.SCALE_FACTOR,
    i[3] = i[3] / OrgChart.SCALE_FACTOR) : !1 === t ? (i[2] = i[2] * OrgChart.SCALE_FACTOR,
    i[3] = i[3] * OrgChart.SCALE_FACTOR) : (i[2] = i[2] / t,
    i[3] = i[3] / t),
    e || (e = [50, 50]),
    ...
    if (i[2] = this.width() / l,
    i[3] = this.height() / l,
    !0 === t && l < this.config.scaleMax || !1 === t && l > this.config.scaleMin || 0 != t && 1 != t && l < this.config.scaleMax && l > this.config.scaleMin) {
        this._hideBeforeAnimation();
        var s = this;
        r ? (clearTimeout(s._timeout),
...
OrgChart.loading.show = function(t) {
    var e = document.createElement("div");
    e.id = "bg-loading",
    e.innerHTML = '<style>@-webkit-keyframes dot-keyframes {0% { opacity: .4; -webkit-transform: scale(1, 1);transform: scale(1, 1);}50% {opacity: 1;-webkit-transform: scale(1.2, 1.2);transform: scale(1.2, 1.2);}100% {opacity: .4;-webkit-transform: scale(1, 1);transform: scale(1, 1);}}@keyframes dot-keyframes {0% {opacity: .4;-webkit-transform: scale(1, 1);transform: scale(1, 1);}50% {opacity: 1;-webkit-transform: scale(1.2, 1.2);transform: scale(1.2, 1.2);}100% {opacity: .4;-webkit-transform: scale(1, 1);transform: scale(1, 1);}}.bg-loading-dots div {margin: 10px;}      .bg-dot-1 {background-color: #039BE5;}.bg-dot-2 {background-color: #F57C00;}.bg-dot-3 {background-color: #FFCA28;}      .bg-loading-dots {text-align: center;width: 100%; position: absolute; top: 0;}.bg-loading-dots--dot {-webkit-animation: dot-keyframes 1.5s infinite ease-in-out;animation: dot-keyframes 1.5s infinite ease-in-out;        border-radius: 10px;display: inline-block;height: 10px;width: 10px;}.bg-loading-dots--dot:nth-child(2) {-webkit-animation-delay: .5s;animation-delay: .5s;}.bg-loading-dots--dot:nth-child(3) {-webkit-animation-delay: 1s;animation-delay: 1s;}</style><div class="bg-loading-dots"><div class="bg-loading-dots--dot bg-dot-1"></div><div class="bg-loading-dots--dot bg-dot-2"></div><div class="bg-loading-dots--dot bg-dot-3"></div></div>',
...
OrgChart.pdfPrevUI.show = function(r, i) {
    ...
    var a, o, n, l = r.element.querySelector("#bg-ppdf-size"), s = r.element.querySelector("#bg-ppdf-layout"), h = r.element.querySelector("#bg-ppdf-scale"), d = r.element.querySelector("#bg-ppdf-margin"), c = r.element.querySelector("#bg-ppdf-header"), g = r.element.querySelector("#bg-ppdf-footer");
    l.value = i.format,
    s.value = i.landscape,
    h.value = i.scale,
    d.value = i.margin,
    c.value = i.header,
    g.value = i.footer,
    OrgChart.anim(r.element.querySelector("#bg-ppdf-btns"), {
        margin: [0, 0, 0, -250]
    }, {
        margin: [0, 0, 0, 0]
...
OrgChart.pdfPrevUI._handler = function(t, e, r) {
    ...
    for (var s = l.querySelector("svg"), h = t.element.querySelector("#bg-ppdf-content"), d = 0; d < a.length; d++) {
        ...
        a[d].backgroundColor && (c.style.backgroundColor = a[d].backgroundColor);
        var p = i.header;
        a[d].header && (p = a[d].header),
        p && (p = p.replace("{current-page}", d + 1).replace("{total-pages}", a.length));
        var f = i.footer;
        a[d].footer && (f = a[d].footer),
        f && (f = f.replace("{current-page}", d + 1).replace("{total-pages}", a.length)),
        a[d].html ? g.write(OrgChart._exportHtml(a[d].html, i, a[d].innerSize.w, a[d].innerSize.h, p, f)) : (s.setAttribute("viewBox", a[d].vb),
        g.write(OrgChart._exportHtml(l.innerHTML, i, a[d].innerSize.w, a[d].innerSize.h, p, f)));
...
OrgChart._exportHtml = function(t, e, r, i, a, o) {
    for (var n = "", l = 0; l < e.margin.length; l++)
        n += e.margin[l] + "px ";
    ...
    return a && (s += '<div id="bg-header" style="width:' + r + "px;color:#757575;position:absolute;left:" + e.margin[3] + 'px;top:0;">' + a + "</div>"),
    s += t,
    o && (s += '<div id="bg-footer" style="width:' + r + "px;color:#757575;position:absolute;left:" + e.margin[3] + 'px;bottom:0;">' + o + "</div>"),
    s += "</div>",
    s += "</body></html>"
}
,
    ...
    for (var r, i, a, o, n, l = "", s = 0; s < this.config.clinks.length; s++) {
        ...
        if ((d.isChildOfGroup || e.res.visibleNodes[d.id]) && ((!d.isChildOfGroup || e.res.visibleNodes[d._groupParentNodeId]) && (c.isChildOfGroup || e.res.visibleNodes[c.id]) && (!c.isChildOfGroup || e.res.visibleNodes[c._groupParentNodeId]))) {
            ...
            if (d.isChildOfGroup) {
                ...
                g += C.x + m.groupPadding[3],
                p += C.y + m.groupPadding[0]
            }
            if (c.isChildOfGroup) {
                C = t.getBGNode(c._groupParentNodeId),
                m = OrgChart.t(C.templateName);
                f += C.x + m.groupPadding[3],
                u += C.y + m.groupPadding[0]
            }
            var O = {}
              , v = {}
              , x = g + d.w / 2
            ...
            h.label && (l += m.label.replace("{x}", I.x).replace("{y}", I.y).replace("{val}", h.label)),
...
OrgChart.prototype._slinks = function(_, k) {
    ...
    function t(t, e, r) {
        ...
        var n = OrgChart.t(t.templateName, I)
        ...
        if (t.level == e.level)
            ...
            for (var h = t; null == a; ) {
                var d = !1
                  ...
                  , g = c.leftNeighbor
                  , p = c.rightNeighbor;
                if (c.id == e.id ? a = c : OrgChart._intersects(c, s, _.config) && (s = OrgChart._addPoint(c, i, _.config, s, o),
                d = !0),
                c.id != e.id) {
                    for (; g; ) {
                        if (g.id == e.id) {
                        ...
                        OrgChart._intersects(g, s, _.config) && (s = OrgChart._addPoint(g, i, _.config, s, o),
                        d = !0),
                        g = g.leftNeighbor
                    }
                    for (; p; ) {
                        ...
                        OrgChart._intersects(p, s, _.config) && (s = OrgChart._addPoint(p, i, _.config, s, o),
                        d = !0),
                        p = p.rightNeighbor
                    }
                }
                if (!d) {
                    var f = i[i.length - 1][0]
                      , u = 0;
                    if (c.parent)
                        switch (l = _.config.levelSeparation,
                        c.parent.layout != OrgChart.mixed && c.parent.layout != OrgChart.tree || (l = _.config.mixedHierarchyNodesSeparation),
                        _.config.orientation) {
                        case OrgChart.orientation.top:
                        case OrgChart.orientation.top_left:
            ...
            switch (n = OrgChart.t(a.templateName, I),
        ...
        r && (i = i.reverse()),
        i[0] = "M" + i[0].join(",");
        for (var v = 1; v < i.length; v++)
            i[v] = "L" + i[v].join(",");
        var x = i.join(" ");
        if (S += '<g c-link-from="{from}" c-link-to="{to}">'.replace("{from}", t.id).replace("{to}", e.id) + n.link.replaceAll("{d}", x),
        N.label) {
            var y = n.label.replace("{x}", m.x).replace("{y}", m.y).replace("{val}", N.label)
              , b = OrgChart._getLabelSize(y)
              , w = -b.height / 2;
            switch (_.config.orientation) {
            case OrgChart.orientation.bottom:
            case OrgChart.orientation.bottom_left:
            ...
            S += n.label.replace("{x}", m.x).replace("{y}", m.y + w).replace("{val}", N.label)
        }
        S += OrgChart.grCloseTag,
        k.content += S
    }
